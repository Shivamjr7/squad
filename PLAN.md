...
- **M22** — Counter-proposals + auto-lock. Schema: `plan_time_proposals` + `plan_time_proposal_votes` + `plans.lock_threshold` column (default 5). For exact-time plans, the original `starts_at` is auto-seeded as the first proposal row; counter-proposals stack on top. Auto-lock rule: `votes` with `in` ≥ `lock_threshold` AND single time+venue plurality → confirm; OR `decide_by` reached → confirm with current plurality (ties = earliest-proposed). Lock email "It's happening — 8:30 at Roxie" to in/maybe voters. "+ Suggest another time" UI on plan detail. "PLAN LOCKS AT 8:30 IF 5+ ARE IN" footer line. (1-2 evenings)
  - **M22.1 — User-configurable lock threshold (Android-launch amendment).** The M22 default of 5 is too high for friend-group sized squads (the most common Squad install pattern). NewPlanForm now exposes a "Locks when" chip row below decide-by — chips of `2+ in / 3+ in / 4+ in / 5+ in / Everyone (N)`. Creator picks; `createPlanSchema.lockThreshold` (optional int) flows to `plans.lock_threshold` at insert; server re-clamps to `min(value, eligibleVoters)` so a stale form can't request an unreachable threshold. Plan-detail and live-ticker copy reads the saved value verbatim. No schema migration — uses the existing M22 column.
