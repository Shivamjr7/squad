...
export const proposeTimeSchema = z
  .object({
    ...
    // so the PLUS/AFTER row can show what the add-on is.
    label: z
      .string()
      .trim()
  ...
  .refine((v) => v.kind !== "addition" || Boolean(v.label?.trim()), {
    message: "Add a short label for the add-on.",
    path: ["label"],
  });
